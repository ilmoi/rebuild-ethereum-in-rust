...
// class Block {
  ...
  //
  //   if (value.length > HASH_LENGTH) { //js peculiarities, may not have this problem in rust. Basically js adds 1 to super large values and turns this into a 65 char string, not 64
  //     return 'f'.repeat(HASH_LENGTH);
  //   }
  //
  //   return '0'.repeat(HASH_LENGTH - value.length) + value; //prepend the value with 0s to make sure it's exactly 64 chars
  // }
  ...
  // static mineBlock({
    ...
    // do {
      // timestamp = Date.now();
      // truncatedBlockHeaders = { //like normal but w/o the nonce value
      //   parentHash: keccakHash(lastBlock.blockHeaders),
      //   beneficiary, //miner's addr
      ...
      // underTargetHash = keccakHash(header + nonce); //hash the two together to see if we get a valid block
    // } while (underTargetHash > target); //note this is doing string comparison - will have to think carefully in rust
